        ...
        with col_res1:
            st.markdown(
                ...
                <div style="padding:25px; border-radius:15px; box-shadow:0 4px 10px rgba(0,0,0,0.1);
                    ...
                    <h2 style="text-align:center; color:{'#2E7D32' if pred_class==1 else '#C62828'};">
                        {'✅ Loan Approved' if pred_class==1 else '❌ Loan Rejected'}
                    </h2>
        ...
        for i, row in df.iterrows():
            with st.expander(f"Applicant #{i+1} — {'Approved ✅' if row['Prediction']==1 else 'Rejected ❌'}"):
                ...
                with colA:
                    ...
                    st.markdown(
                        f"**Confidence:** {row['Prob_Approved']:.1f}% approved | {row['Prob_NotApproved']:.1f}% not approved"
                    )
                with colB:
